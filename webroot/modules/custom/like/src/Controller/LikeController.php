...
class LikeController extends ControllerBase {
  ...
  public function handler($clicked, $data) {
    ...
    if ($user == 0) {
      ...
      $user_login_register = $this->like_login_register();
    ...
    if ($clicked == 'like') {
      if (!$already_clicked) {
        ...
        return $this->like_status($response);
      }
      $return = $response->addCommand(
   ...
   * @return mixed
   */
  protected function like_login_register() {
    $options = array(
      'attributes' => array(
        'class' => array(
          'use-ajax',
   ...
   * @return AjaxResponse
   */
  protected function like_status(AjaxResponse $response) {
    return $response->addCommand(
      new HtmlCommand('#like_status', 'Already liked/disliked..!')
    );
  }
